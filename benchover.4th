...
dim   1 * floats allocate throw constant b b 1   dim 30000 init-matrix
dim   1 * floats allocate throw constant c

: bench
    20 0 do
	a b c dim dim 1 matmulr
    loop ;
bench
...
[defined] f~ [if]
    c 1 dim checksum sum f@ 945305864500000. d>f -1e-5 f~ 0= [if]
	.( result incorrect ) cr
    [else]
	.( result correct) cr
    [then]
